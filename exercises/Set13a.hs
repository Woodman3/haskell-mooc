...
split s
  | ' ' `elem` s = Just (takeWhile (/=' ') s, tail (dropWhile (/=' ') s))
  | otherwise = Nothing
...
balance accountName = BankOp balanceHelper
  where balanceHelper (Bank accounts) = (Map.findWithDefault 0 accountName accounts, Bank accounts)
...
count x = do
  state <- get
  let (found,rest) = partition ((==x) . fst) state
  case found of
    [] -> put ((x,1):rest)
    [(x,n)] -> put ((x,n+1):rest)
