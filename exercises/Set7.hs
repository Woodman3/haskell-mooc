...
passwordAllowed xs (MinimumLength n) = length xs >= n
passwordAllowed xs (ContainsSome ys) = any (`elem` ys) xs
passwordAllowed xs (DoesNotContain ys) = not (any (`elem` ys) xs)
passwordAllowed xs (And x y) = passwordAllowed xs x && passwordAllowed xs y
passwordAllowed xs (Or x y) = passwordAllowed xs x || passwordAllowed xs y
