...
incrementKey key (x:xs)
    |   fst x == key = (key, snd x + 1) : incrementKey key xs
...
freqs = foldr (Map.alter (\y -> case y of
    Just z -> Just (z+1)
    Nothing -> Just 1)) Map.empty
...
transfer from to amount bank = case (Map.lookup from bank, Map.lookup to bank, amount > 0, Map.findWithDefault 0 from bank >= amount) of
    (Just _, Just _, True, True) -> Map.adjust (+ amount) to $ Map.adjust (subtract amount) from bank
    _ -> bank
...
maxIndex =  fst . foldr1 (\x y -> if snd x > snd y then x else y) . assocs 
